...
> **Summary:** Every source used to build RECPT — standards, frameworks, research papers, datasets, tools, and project documents. For each source: what it is, what it was used for, which part of the app or methodology it influenced, and its confirmation status.
...
- "Which parts of RECPT are based on published standards?"
...
| **How RECPT uses it** | The AI inference carbon formula is SCI-for-AI conformant: `tokens → energy → carbon` with location-based grid intensity and no offsets. The Prove > Methodology page explicitly lists SCI-for-AI as the primary standard. |
| **Where it appears** | `docs/research/recpt-calculation-methodology.md`; `app.py` Prove page; `docs/prd/recpt-prd.md` |
| **Caveats** | RECPT implements SCI-for-AI for the operational carbon (O) dimension. The embodied carbon (M) dimension is not yet implemented (Boavizta integration is planned). |
...
| **How RECPT uses it** | The core formula `(tokens / 1M) × kWh_per_1M_tokens × grid_gCO₂e_per_kWh` follows the SCI-for-AI approach for inference carbon. RECPT's methodology is described as "SCI-for-AI conformant" in the app. |
| **Where it appears** | `app.py` (Prove > Methodology > Standards Alignment table); `docs/research/recpt-calculation-methodology.md` Section 11 |
| **Caveats** | RECPT implements inference carbon only. Training and fine-tuning carbon is out of scope for the MVP. |
...
| **How RECPT uses it** | RECPT's methodology is described as "ISO 21031 conformant" — location-based, no offsets, transparent assumptions. |
| **Where it appears** | `docs/research/recpt-calculation-methodology.md`; `app.py` Prove page caption |
...
| **How RECPT uses it** | (1) The cloud carbon formula `usage_kWh × grid_gCO₂e/kWh` mirrors CCF's approach. (2) The cloud usage CSV schema (`date, app, service, region, usage_kwh, cost_usd`) is shaped like a CCF billing export. (3) The CCF output schema is listed in the Standards Alignment table. (4) The RECPT repo is a fork of the CCF codebase. (5) The `carbon_factors.csv` values are labelled "CCF-v2.1 methodology." |
...
| **Caveats** | The CCF TypeScript codebase is present in the `packages/` folder but is not invoked at runtime by the RECPT MVP Python app. The relationship is methodological and structural, not a live code dependency. |
...
| **How RECPT uses it** | The `llm_usage.csv` data is "Langfuse-shaped" — it follows the field structure of a Langfuse export. The Connect page's Langfuse connector form uses Langfuse's real API credential fields. The `llm_trace_export.json` data is shaped like Langfuse trace exports. |
...
| **How RECPT uses it** | RECPT uses OpenTelemetry GenAI conventions for token field naming in the trace data schema. Listed in the Standards Alignment table. |
...
| **How RECPT uses it** | `semgrep_findings.json` follows the Semgrep JSON output format. The Energy Debt tab reads this file to count findings per app. Listed in the Standards Alignment table. |
...
| **How RECPT uses it** | The grid intensity values in `grid_intensity.csv` are sourced from Electricity Maps data (along with EPA eGRID and ENTSO-E for regional cross-reference). |
| **Where it appears** | `docs/sample-data/carbon_factors.csv` (labelled as source); `docs/research/recpt-calculation-methodology.md` Section 11 |
...
| **How RECPT uses it** | US regional grid intensity values (us-east: 380, us-west: 210) are aligned to EPA eGRID data. |
| **Where it appears** | `docs/research/recpt-calculation-methodology.md` Section 11; `docs/sample-data/carbon_factors.csv` |
...
| **How RECPT uses it** | EU regional grid intensity value (eu-west: 290 gCO₂e/kWh) is aligned to ENTSO-E data. |
| **Where it appears** | `docs/research/recpt-calculation-methodology.md` Section 11 |
...
| **How RECPT uses it** | The kWh/1M token coefficient derivation cites this paper as a primary source for inference energy estimation. |
| **Where it appears** | `docs/research/recpt-calculation-methodology.md` Sections 5 and 11 |
...
| **How RECPT uses it** | Provides context for RECPT's focus on inference carbon (run-time) rather than training carbon (build-time). |
| **Where it appears** | `docs/research/recpt-calculation-methodology.md` Section 11 |
...
| **How RECPT uses it** | Token throughput figures (500–1,500 tokens/second for large models on A100) used in the coefficient derivation in `recpt-calculation-methodology.md`. |
| **Where it appears** | `docs/research/recpt-calculation-methodology.md` Section 5 |
...
| **How RECPT uses it** | Step 1 of the coefficient derivation — GPU power draw is the starting point for the kWh/token calculation. |
| **Where it appears** | `docs/research/recpt-calculation-methodology.md` Section 5 |
...
| **How RECPT uses it** | The water formula `energy_kWh × WUE_liters_per_kWh` follows The Green Grid's WUE definition. |
| **Where it appears** | `docs/research/recpt-calculation-methodology.md` Sections 8 and 11 |
...
| **How RECPT uses it** | Provides the research basis for including water as a RECPT metric. Informs the regional WUE estimates. |
| **Where it appears** | `docs/research/recpt-calculation-methodology.md` Sections 8 and 11 |
...
| **How RECPT uses it** | The eu-west WUE value (0.5) is described as "consistent with Google's reported EU West figures." |
| **Where it appears** | `docs/research/recpt-calculation-methodology.md` Section 2 |
...
| **How RECPT uses it** | Informs the PUE assumption (≈1.2) used in the coefficient derivation, and provides context for WUE regional estimates. |
| **Where it appears** | `docs/research/recpt-calculation-methodology.md` Sections 5 and 11 |
...
| **How RECPT plans to use it** | Future integration to add the embodied carbon (M) dimension to the SCI score, upgrading from Scope 2 operational only. |
...
| **How it would improve RECPT** | Would allow RECPT to expand from three model classes to named model coefficients with benchmark-backed values. |
| **Note** | Referenced in `docs/research/feasibility-ai-carbon-product.md` but not formally cited in `recpt-calculation-methodology.md`. |

---

## Internal project documents

### RECPT Project Strategy Analysis
| Attribute | Detail |
|---|---|
...
| **How it shaped RECPT** | The pivot from an AI:works-embedded tool to an independent, sellable product is documented here. The "build green + prove it" two-layer product vision originated in this document. |

---

### RECPT Feasibility Analysis
| Attribute | Detail |
|---|---|
...
| **How it shaped RECPT** | Defined the three data tiers (cloud billing, gateway logs, GPU telemetry), the confidence level framework (High/Medium/Low), and the reference architecture used in the MVP. |

---

### RECPT Calculation Methodology
| Attribute | Detail |
|---|---|
| **Type** | Internal technical reference |
| **File** | `docs/research/recpt-calculation-methodology.md` |
| **Status** | ✅ Confirmed in app |
...
| **How it shaped RECPT** | This is the single authoritative reference for all calculations in `app.py`. The code implements exactly what this document specifies. |

---

## Key takeaways

- The core RECPT methodology rests on well-established published sources: SCI-for-AI (ISO 21031), Electricity Maps/EPA eGRID grid intensity, and Luccioni et al. for inference energy
...
- The CCF codebase and methodology are the structural and intellectual ancestors of RECPT's cloud carbon layer
