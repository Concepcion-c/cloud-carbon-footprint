# 05 — Cloud Carbon Footprint & RECPT

> **Summary:** What Cloud Carbon Footprint (CCF) is, how RECPT relates to it, what was reused versus built from scratch, and the path for RECPT to eventually grow into a proper CCF extension. Written in plain English.
...
**What CCF does not do:** CCF measures cloud infrastructure — the servers and services. It does not measure AI inference carbon. When you call the OpenAI API or run a Claude model on Bedrock, CCF does not see those tokens or calculate their energy and carbon footprint. That gap is exactly what RECPT is designed to fill.

---

## What is the relationship between RECPT and CCF?

The RECPT repository is a **fork of the CCF codebase**. This means that when the project started, Thoughtworks took a complete copy of the CCF source code and began building on top of it.
...
| **CCF code running in the MVP?** | No — the RECPT Python app does not invoke any CCF code at runtime |
| **CCF methodology used?** | Yes — RECPT borrows CCF's carbon calculation approach for cloud infrastructure |
| **CCF data formats used?** | Yes — the cloud usage data in RECPT is shaped like CCF's billing export format |
| **CCF grid intensity values?** | Yes — the regional gCO₂e/kWh values in RECPT are sourced from the same public datasets CCF uses (Electricity Maps, EPA eGRID, ENTSO-E), and the values are labelled as "CCF-v2.1" methodology |

The simplest way to explain this: **RECPT borrows CCF's ideas and data; the code is waiting to be connected.**

---

## The microwave analogy

Think of CCF as a fully equipped professional kitchen. RECPT is currently a microwave sitting on the counter next to it.

- The microwave (RECPT MVP) works great and does exactly what it was designed to do
- It is not using the kitchen (the CCF codebase)
- The long-term plan is to actually cook in the kitchen — to wire RECPT into CCF's existing infrastructure so the two become one product
...
## What CCF already provides that RECPT would use in production

CCF has a modular architecture with well-defined extension points:

| CCF component | What it does | How RECPT would use it |
|---|---|---|
| `@cloud-carbon-footprint/core` | Core estimation interfaces and algorithms | RECPT would add a new `LLMInferenceEstimator` implementing CCF's `IFootprintEstimator` interface |
| `@cloud-carbon-footprint/aws` | AWS billing connectors and service estimators | RECPT would add Bedrock (AWS's AI service) as a new `ICloudService` implementation |
| `@cloud-carbon-footprint/gcp` | Google Cloud connectors | RECPT would add Vertex AI as a new service |
| `@cloud-carbon-footprint/azure` | Azure connectors | RECPT would add Azure OpenAI as a new service |
| `@cloud-carbon-footprint/api` | REST API server | RECPT would add new endpoints for AI footprint data |
| `@cloud-carbon-footprint/client` | React dashboard | RECPT would add the Observe/Optimize/Prove pages |
...
## What RECPT built that CCF does not have

Everything RECPT does with AI inference is net new — not present in any form in the existing CCF codebase:
...
## What RECPT directly borrowed from CCF

| Borrowed element | How it appears in RECPT |
|---|---|
...
| **Location-based, no offsets principle** | CCF's philosophical position — that location-based grid intensity with no market-based offsets is the honest approach — is directly adopted by RECPT |

---

## The full production path

For RECPT to become a proper CCF extension (rather than a parallel Python app), the engineering work would proceed in roughly four phases:
...
- CCF is Thoughtworks' existing open-source cloud carbon tool, and RECPT is its natural evolution into the AI era
- The RECPT MVP is currently a parallel Python app inspired by CCF's methodology, not an extension of CCF's code
